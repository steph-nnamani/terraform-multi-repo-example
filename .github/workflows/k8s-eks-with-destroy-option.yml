    ...
    inputs:
      action:
        ...
        options:
          ...
          - destroy
      confirm_destroy:
        description: 'Type "destroy" to confirm destruction (only needed for destroy action)'
        required: false
  push:
    branches:
      - destroy
    paths:
      - 'stage/services/kubernetes-eks/**'

jobs:
  terraform-action:
    runs-on: ubuntu-latest
    timeout-minutes: 60  # Increase timeout for EKS operations
    ...
    steps:
      ...
      - name: Validate destroy confirmation
        ...
        run: |
          echo "Error: You must type 'destroy' in the confirm_destroy field to proceed with destruction"
          exit 1
